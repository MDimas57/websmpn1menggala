...
<style>
    ...
    .profil-content ul, .profil-content ol {
        margin-bottom: 1.5rem !important;
    ...
    .profil-content strong, .profil-content b {
    ...
    <div class="container relative z-10 px-4 pb-24 mx-auto pt-36 max-w-7xl">
        ...
        @if($profil)
            <div class="grid items-start grid-cols-1 gap-8 lg:grid-cols-12">
                ...
                {{-- A. KOLOM KIRI: IDENTITAS & STATISTIK --}}
                    ...
                    <div class="bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden relative group">
                        ...
                        <div class="relative p-8 text-center">
                            <div class="relative w-40 h-40 mx-auto mb-6">
                                ...
                                <div class="relative w-full h-full p-4 bg-white border-4 rounded-full shadow-lg border-slate-800">
                                    @if($profil->logo)
                                        <img src="{{ asset('storage/' . $profil->logo) }}" alt="Logo" class="object-contain w-full h-full">
                                    @else
                            ...
                            <h2 class="mb-2 text-2xl font-bold leading-tight text-white">
                                {{ $profil->nama_sekolah ?? 'SMP N 1 Menggala' }}
                            </h2>
                        ...
                        <div class="grid grid-cols-3 border-t divide-x divide-slate-800 border-slate-800 bg-slate-900/50 backdrop-blur-sm">
                            <div class="p-4 text-center transition-colors hover:bg-slate-800 group">
                                <p class="text-2xl font-black text-white group-hover:text-yellow-400">{{ $profil->jumlah_peserta_didik ?? 0 }}</p>
                                <p class="text-[10px] uppercase tracking-wider text-slate-400 font-bold mt-1">Siswa</p>
                            </div>
                            <div class="p-4 text-center transition-colors hover:bg-slate-800 group">
                                <p class="text-2xl font-black text-white group-hover:text-yellow-400">{{ $profil->jumlah_tenaga_pendidik ?? 0 }}</p>
                                <p class="text-[10px] uppercase tracking-wider text-slate-400 font-bold mt-1">Guru</p>
                            </div>
                            <div class="p-4 text-center transition-colors hover:bg-slate-800 group">
                                <p class="text-2xl font-black text-white group-hover:text-yellow-400">{{ $profil->jumlah_rombel ?? 0 }}</p>
                ...
                {{-- B. KOLOM KANAN: DESKRIPSI (VISI & MISI) --}}
                <div class="lg:col-span-8">
                    <div class="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-8 md:p-12 relative overflow-hidden">
                        ...
                        <div class="relative z-10">
                            ...
                            {{-- Wrapper profil-content agar CSS di atas bekerja --}}
                            <div class="text-justify text-slate-600 profil-content">
                                {!! $profil->deskripsi !!}
                            </div>
                        </div>
