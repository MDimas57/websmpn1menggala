...
--}}
    ...
    <div class="relative w-full min-h-screen overflow-hidden font-sans bg-slate-50">
        ...
        <div class="absolute inset-0 pointer-events-none">
            ...
            <div
                class="absolute top-0 right-0 w-[600px] h-[600px] bg-blue-100 rounded-full blur-3xl opacity-60 -translate-y-1/2 translate-x-1/2">
            </div>
            <div
                class="absolute bottom-0 left-0 w-[500px] h-[500px] bg-yellow-100 rounded-full blur-3xl opacity-60 translate-y-1/3 -translate-x-1/4">
            </div>
        </div>
    ...
    --}}
        <div class="container relative z-10 px-4 pb-24 mx-auto pt-36 max-w-7xl">
            ...
            <div class="flex flex-col items-center mb-12 text-center">
                <a href="{{ url()->previous() }}"
                    class="px-4 py-1.5 rounded-full bg-yellow-100 text-yellow-700 text-xs font-bold tracking-widest uppercase mb-4 border border-yellow-200 hover:bg-yellow-200 transition-colors">
                    &larr; Kembali
                </a>
            </div>
            ...
            --}}
                <div class="lg:col-span-8">
                    <article
                        class="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-8 md:p-12 overflow-hidden relative">

                        {{-- Header Artikel --}}
                        <header class="mb-10 text-center md:text-left">
                            <div class="flex items-center justify-center gap-2 mb-6 md:justify-start">
                                <span
                                    class="px-3 py-1 text-xs font-bold tracking-wider text-blue-600 uppercase rounded-lg bg-blue-50">Berita
                                    Sekolah</span>
                                <span class="text-slate-300">•</span>
                                <span
                                    class="text-xs font-bold tracking-wider uppercase text-slate-500">{{ $berita->created_at->format('d F Y') }}</span>
                            </div>
                            ...
                            {{-- Meta Penulis --}}
                            <div
                                class="flex items-center justify-center gap-4 p-4 border md:justify-start rounded-2xl bg-slate-50 border-slate-100 w-fit">
                                <div
                                    class="flex items-center justify-center w-12 h-12 text-xl font-bold bg-yellow-400 rounded-full shadow-md text-slate-900">
                                    {{ substr($berita->penulis ?? 'A', 0, 1) }}
                                </div>
                                <div class="text-left">
                                    <p class="text-sm font-bold text-slate-900">{{ $berita->penulis ?? 'Administrator' }}
                                    </p>
                        ...
                        {{-- Isi Konten --}}
                        <div
                            class="mb-12 leading-relaxed prose prose-lg text-justify prose-slate max-w-none text-slate-600 prose-p:mb-4 prose-li:mb-2 prose-ul:my-4">
                            {!! $berita->konten !!}
                        </div>
                        ...
                        <div class="pt-8 mt-8 border-t border-slate-100">
                            <p
                                class="mb-4 text-sm font-bold tracking-widest text-center uppercase md:text-left text-slate-400">
                                Bagikan Artikel Ini</p>

                            <div class="flex flex-wrap justify-center gap-3 md:justify-start">
                                {{-- Facebook --}}
                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->fullUrl()) }}"
                                    target="_blank"
                                    class="flex items-center gap-2 px-5 py-2.5 bg-[#1877F2] text-white rounded-full font-bold hover:shadow-lg hover:-translate-y-1 transition-all">
                                ...
                                <a href="https://wa.me/?text={{ urlencode($berita->judul . ' ' . request()->fullUrl()) }}"
                                    target="_blank"
                                    class="flex items-center gap-2 px-5 py-2.5 bg-[#25D366] text-white rounded-full font-bold hover:shadow-lg hover:-translate-y-1 transition-all">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path
                                ...
                                {{-- Copy Link --}}
                                <button id="copyBtn"
                                    class="flex items-center gap-2 px-5 py-2.5 bg-slate-100 text-slate-600 rounded-full font-bold hover:bg-slate-200 transition-colors">
                                ...
                                <span id="copyMsg"
                                    class="hidden px-3 py-2 text-sm font-bold text-green-600 rounded-lg bg-green-50 animate-fade-in-up">Link
                                    Tersalin!</span>
                            </div>
                        </div>
                    </article>
            ...
            --}}
                <aside class="lg:col-span-4 lg:sticky lg:top-32">
                    <div class="bg-slate-900 rounded-[2.5rem] p-6 shadow-2xl overflow-hidden relative">
                        ...
                        {{-- Judul Sidebar --}}
                        <h3
                            class="flex items-center gap-3 pb-4 mb-6 text-xl font-bold text-white border-b border-slate-700">
                            ...
                            @if (isset($unreadBeritas) && $unreadBeritas->count())
                                ...
                                @forelse($others as $u)
                                    <a href="{{ route('berita.show', $u->slug) }}"
                                        class="flex gap-4 p-2 transition-colors group rounded-xl hover:bg-white/5">
                                        {{-- Thumbnail Kecil --}}
                                        <div class="relative w-20 h-20 overflow-hidden rounded-lg shrink-0 bg-slate-800">
                                            @if ($u->gambar)
                                                <img src="{{ asset('storage/' . $u->gambar) }}" alt="{{ $u->judul }}"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <h4
                                                class="mb-1 text-sm font-bold leading-snug transition-colors text-slate-200 group-hover:text-yellow-400 line-clamp-2">
                                                {{ $u->judul }}
                                            </h4>
